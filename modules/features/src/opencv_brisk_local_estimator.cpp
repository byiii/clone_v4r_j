...
#include <v4r/features/opencv_brisk_local_estimator.h>
...
bool
OpenCVBRISKLocalEstimation<PointT>::estimate (const pcl::PointCloud<PointT> & in,
                                              ...
                                              std::vector<std::vector<float> > & signatures)
{
    ...
    brisk_->operator ()(grayImage, cv::Mat(), ks, descriptors, false);

    //use indices_ to check if the keypoints and feature should be saved
    //compute BRISK keypoints and BRISK features
    //backproject BRISK keypoints to 3D and save in keypoints
    //save signatures
    signatures.resize (ks.size (), std::vector<float>(64));
    ...
    indices_.clear();
    std::cout << "Number of SURF features:" << kept << std::endl;

    return true;
}

template class V4R_EXPORTS OpenCVBRISKLocalEstimation<pcl::PointXYZRGB>;
}
