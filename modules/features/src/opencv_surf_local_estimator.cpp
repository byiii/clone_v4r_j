...
#include <v4r/features/opencv_surf_local_estimator.h>
...
bool
OpenCVSURFLocalEstimation<PointT>::estimate (const pcl::PointCloud<PointT> & in,
                                             ...
                                             std::vector<std::vector<float> > & signatures)
{
    ...
    surf_->operator ()(grayImage, cv::Mat(), ks, descriptors, false);

    //use indices_ to check if the keypoints and feature should be saved
    //compute SURF keypoints and SURF features
...
template class V4R_EXPORTS OpenCVSURFLocalEstimation<pcl::PointXYZRGB>;
}
